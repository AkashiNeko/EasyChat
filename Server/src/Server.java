    ...
    public void start() { 
        while (!stopServer) {
            try {
                ...
                Logger.print("\033[32mclient connected\033[0m");
                BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(socket.getInputStream()));
                PrintWriter printWriter = new PrintWriter(socket.getOutputStream(), true);
                String msg = bufferedReader.readLine();
                if (msg != null) {
                    Logger.print("port:" + port + " \"" + msg + "\"");
                ...
                Logger.print("server error: " + e.getMessage());
            } finally {
                try {
                    socket.close();
                } catch (IOException e) {
                    Logger.print("client close failed: " + e.getMessage());
                }
                Logger.print("\033[33mclient closed\033[0m");
            }
        }
        try {
            serverSocket.close();
