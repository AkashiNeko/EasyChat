...
public class UserDAO {
    ...
    public UserDAO() {
        try {
            ...
            Logger.print("Database connected: UserName=" + DB_USER);
        } catch (Exception e) {
            Logger.print("Database connect failed: " + e.getMessage());
            System.exit(-1);
        }
    }
    ...
    public String deleteFriend(String user1, String user2) throws SQLException {
        ...
        if (count1 > 0) {
            ...
            if (ret == 1) {
                Logger.print("delete friendship: \"" + user1 + "\" and \"" + user2 + "\"");
                // 转发
                synchronized (UserThread.onlineUsers) {
                    if (UserThread.isOnline(user2)) {
                        PrintWriter printWriter = UserThread.onlineUsers.get(user2);
    ...
    public String sendMessage(String username, String target, String base64msg) throws SQLException {
        ...
        String tableName = "chat_" + eid1 + "_" + eid2;
        if (this.tableNotExists(tableName))
            tableName = "chat_" + eid2 + "_" + eid1;
        PreparedStatement statement = connection.prepareStatement(
        ...
        Logger.print("\033[35m" + username + ": \"" + msg + "\"\033[0m");
